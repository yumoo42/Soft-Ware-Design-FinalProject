...
class CTBertWordEmbedding(nn.Module):
    def __init__(self,
        vocab_size,
        hidden_dim,
        vocab_dim,
...
class CTBertNumEmbedding(nn.Module):
...
class CTBertFeatureProcessor(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.word_embedding = CTBertWordEmbedding(
...
class CTBertTransformerLayer(nn.Module):
...
class CTBertInputEncoder(nn.Module):
    def __init__(self,
        feature_extractor,
        feature_processor,
        device=dev,
...
class CTBertEncoder(nn.Module):
    def __init__(self,
        ...
        if num_layer > 1:
            encoder_layer = CTBertTransformerLayer(
                d_model=hidden_dim,
                nhead=num_attention_head,
                dropout=hidden_dropout_prob,
                dim_feedforward=ffn_dim,
...
class CTBertLinearClassifier(nn.Module):
    def __init__(self,
        num_class,
        hidden_dim=128) -> None:
        super().__init__()
...
class CTBertLinearRegression(nn.Module):
    def __init__(self,
        hidden_dim=128) -> None:
        super().__init__()
        self.fc = nn.Linear(hidden_dim, 1)
...
class CTBertProjectionHead(nn.Module):
    def __init__(self,
        hidden_dim=128,
        projection_dim=128):
        super().__init__()
...
class CTBertCLSToken(nn.Module):
    def __init__(self, hidden_dim) -> None:
        super().__init__()
...
class TableGPTMaskToken(nn.Module):
...
class CTBertModel(nn.Module):
    def __init__(self,
        ...
        **kwargs,
        ...
        feature_processor = CTBertFeatureProcessor(
        ...
        self.input_encoder = CTBertInputEncoder(
            feature_extractor=feature_extractor,
            feature_processor=feature_processor,
            device=device,
        )

        self.encoder = CTBertEncoder(
            hidden_dim=hidden_dim,
            num_layer=num_layer,
        ...
        self.cls_token = CTBertCLSToken(hidden_dim=hidden_dim)
        self.device = device
        self.to(device)
...
class CTBertRegression(CTBertModel):
    def __init__(self,
        categorical_columns=None,
        numerical_columns=None,
        binary_columns=None,
        ...
        **kwargs,
        ...
        self.res = CTBertLinearRegression(hidden_dim=hidden_dim)
    ...
    def forward(self, x, y=None, table_flag=0):
        ...
        if y is not None:
            if isinstance(y, pd.Series):
                ...
                y = torch.tensor(y, device=logits.device, dtype=torch.float32)
...
class CTBertForCL(CTBertModel):
    def __init__(self,
        ...
        **kwargs,
        ...
        self.projection_head = CTBertProjectionHead(hidden_dim, projection_dim)
...
class TableGPTForMask(CTBertModel):
    def __init__(self,
        categorical_columns=None,
        numerical_columns=None,
        binary_columns=None,
        ...
        **kwargs,
        ...
        self.num_projection_head = CTBertProjectionHead(hidden_dim, 1)
        self.cat_projection_head = CTBertProjectionHead(hidden_dim, hidden_dim)
        self.mask_token = TableGPTMaskToken(hidden_dim)
    ...
    def forward(self, x, y=None, table_flag=0):
        if isinstance(x, dict):
            ...
            raise ValueError(f'TableGPTForMask takes inputs with dict or pd.DataFrame, find {type(x)}.')
        ...
        if num_encoder_output.nelement() != 0:
            num_encoder_output = self.num_projection_head(num_encoder_output)
        ...
        if cat_encoder_output.nelement() != 0:
            cat_encoder_output = self.cat_projection_head(cat_encoder_output)
    ...
    def mask_features(self, attention_mask, num_cnt, cat_cnt):
        if num_cnt==0 or cat_cnt==0:
            masked_indices = torch.bernoulli(torch.full(attention_mask.shape, self.mlm_probability))
        else:
            num_mp = min(1.0, self.mlm_probability*3*(1+cat_cnt/num_cnt)/10)
            cat_mp = min(1.0, self.mlm_probability*7*(1+num_cnt/cat_cnt)/10)
    ...
    def minmax_norm(self, emb, eps=1e-12):
