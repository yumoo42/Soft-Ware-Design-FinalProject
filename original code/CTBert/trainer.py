    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        self.trainloader_list = [
            self._build_dataloader((trainset,dataindex), batch_size, collator=self.collate_fn, num_workers=num_workers) for dataindex,trainset in enumerate(train_set_list)
        ]
        if test_set_list is not None:
            self.testloader_list = [
                self._build_dataloader((testset,dataindex), eval_batch_size, collator=self.collate_fn, num_workers=num_workers, shuffle=False) for dataindex,testset in enumerate(test_set_list)
            ]
        else:
